...
```jsx title="src/index.jsx" showLineNumbers=40
        },
      },
      // diff-add-start
      Catch: {
...
Open `src/pages/CatchLog.jsx` and locate the `handleAddCatch` function. Add the following the create method to save the new catch data to the RADFish Catch collection.

```jsx title="src/pages/CatchLog.jsx" showLineNumbers=280
...
**1. Accessing the Catch Collection:**
```jsx
const Catch = app.stores["trip"].getCollection("Catch");
```
This line retrieves the Catch collection from RADFish's trip store. The `app.stores["trip"]` accesses our trip store (defined in `src/index.jsx`), and `getCollection("Catch")` gets the specific Catch collection we defined in our schema.

**2. Creating the Data Object:**
```jsx
const newCatchData = {
  ...currentCatch,           // Spread all form input values
  id: crypto.randomUUID(),   // Generate unique ID
  tripId: tripId,           // Link to parent trip
  weight: Number(currentCatch.weight),    // Convert to number
  length: Number(currentCatch.length),    // Convert to number
  // Handle optional coordinates...
  createdAt: new Date().toISOString(),    // Add timestamp
};
```
We create a new object that combines the user's form inputs (`currentCatch`) with additional metadata. The spread operator (`...currentCatch`) copies all form values, then we add/override specific fields like converting weight/length to numbers and generating a unique ID.

**3. Saving to RADFish:**
```jsx
await Catch.create(newCatchData);
```
The `Catch.create()` method saves our data object to IndexedDB through RADFish. This method validates the data against our schema, ensures required fields are present, and stores it persistently. We use `await` because this is an asynchronous database operation.
...
```jsx title="src/pages/CatchLog.jsx" showLineNumbers=305
        setCatches((prev) => [newCatchData, ...prev]);
