...
- **`FormGroup`**: Wrapper that groups related form elements (label, input, hints, errors) for proper spacing and accessibility. Each input field needs its own FormGroup wrapper.
...
- `defaultValue={tripData.startTime}` binds the input to the `startTime` field in our state
...
- `value={tripData.startWeather}` binds the selected option to the `startWeather` field in the state.
