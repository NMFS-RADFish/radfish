...
```js title="vite.config.js" showLineNumbers=55
server: {
  open: true,
  port: 3000,
  proxy: {
...
### 3.1: Update the Submission Handler

Now let's modify the `handleSubmit` function to include both trip and catch data in the server submission. Note that the `catches` state variable is already available from Lesson 6, where we added it to store catch data for API submission:
...
**Understanding the Enhanced Logic:**

1. **Offline Path**: Simple local save with "Not Submitted" status
2. **Online Path**:
...
3. **Error Handling**: Provides detailed feedback about server submission failures
