...
```jsx title="src/pages/ReviewSubmit.jsx" showLineNumbers=208
if (isOffline) {
  //diff-remove-start
  // Offline: Save status as "Not Submitted" locally
  // We'll implement this in the next step
...
```jsx title="src/pages/ReviewSubmit.jsx" showLineNumbers=219
} else {
  //diff-remove-start
  // Online: Attempt to submit to the backend
  // We'll implement this in subsequent steps
...
```jsx title="src/pages/ReviewSubmit.jsx" showLineNumbers=224
// Prepare submission data with trip and associated catches
const submissionData = {
  trip: trip,
  catches: catches,
...
```jsx title="src/pages/ReviewSubmit.jsx" showLineNumbers=246
if (!response.ok) {
  const status = response.status;
  const statusText = response.statusText
    ? response.statusText.trim()
