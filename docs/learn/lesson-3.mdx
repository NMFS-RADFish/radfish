...
- **`collections`**: Contains all the different types of data within this store (we'll add a `CatchForm` collection later for fish catch records). The `TripForm` collection is used to store the trip form data.
...
function StartTrip() {
  ...
  // State management - existing formData state from lesson 2
...
- **`Form.find({ id: tripId })`**: Queries the collection for records matching the provided criteria. This returns an array of matching trips. We use `find()` instead of `findById()` as it's the standard RADFish collection method.
...
- **Update Path**: If `tripId` exists, we call `Form.update()` with the existing ID and our `tripData`. RADFish finds the record in IndexedDB and updates it with the new values.
...
- **Direct RADFish Access**: You've learned to work directly with RADFish stores and collections using `app.stores["trip"].getCollection("Form")`
