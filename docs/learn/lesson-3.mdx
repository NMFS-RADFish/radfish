...
```
:::info
**Understanding Constants for Store and Collection Names:**

The `../utils/constants` file provides centralized constants for store and collection names throughout the application:

- **`STORE_NAMES.TRIP_STORE`**: References the main data store name ("tripStore")
- **`COLLECTION_NAMES.TRIP_COLLECTION`**: References the trip data collection name ("TripCollection")
- **`COLLECTION_NAMES.CATCH_COLLECTION`**: References the catch data collection name ("CatchCollection"). We'll add this collection later in lesson 4.

Using constants instead of hardcoded strings provides several benefits:
- **Maintainability**: Changes to store/collection names only need to be updated in one place
- **Type Safety**: Reduces typos and runtime errors from mistyped strings
- **Consistency**: Ensures all components use the same naming conventions
:::
...
- **`tripStore.getCollection(COLLECTION_NAMES.TRIP_COLLECTION)`**: Within the `tripStore` store, accesses the collection named "TripCollection". Collections are analogous to tables in a relational database or collections in MongoDB; they hold the actual data records (in this case, our trip forms).
...
// Already declared for you in lesson 2
const navigateHome = () => {
  navigate("/");
};
...
These functions provide the same navigation capabilities we'll later see abstracted into custom hooks in lesson 4.
...
- **Update Path**: If `tripId` exists, we call `tripCollection.update()` with the existing ID and our `tripData`. RADFish finds the record in IndexedDB and updates it with the new values.
- **Create Path**: If no `tripId`, it's a new trip:
  - Generate a unique ID using `crypto.randomUUID()`
  - Call `tripCollection.create()` with the new ID and our `tripData`
...
- We use the `navigateWithTripId()` function we created in section 1.4 to go to the `/catch` route
