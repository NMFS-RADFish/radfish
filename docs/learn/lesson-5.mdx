...
Before we can implement validation, we need to add state to track validation errors and form submission status. Add these states after your existing `formData` state:
...
- **Validation First**: `validateForm(formData)` runs before any database operations
...
### 2.5: Add Error States to Form Components

Now we need to update the form fields to display validation errors. We'll add error states to the FormGroup, Label, and input components so they visually indicate when validation fails.

Let's start by updating the Date field to include error states:
...
Now apply the same error state pattern to the Time and Weather fields:
...
- **FormGroup Error State**: `error={submitted && errors.tripDate}` applies USWDS error styling only after submission attempt
- **Label Error Integration**: `error={submitted && errors.tripDate}` on the Label component provides visual error indication
- **Input Validation Status**: `validationStatus={submitted && errors.tripDate ? "error" : undefined}` adds error styling to the input
- **Submitted Check**: The `submitted &&` pattern ensures errors only display after the user attempts to submit, not while initially typing
- **ARIA Accessibility**: `aria-describedby` links the input to both hint text and error messages for screen readers

### 2.6: Import and Add Error Messages

To display the actual error text to users, we need to import the `ErrorMessage` component from USWDS and add it to each form field.
...
**Then, add the ErrorMessage component to each form field:**
...
- **Conditional Rendering**: `{submitted && errors.tripDate && (...)}` ensures error messages only appear after submission attempt
- **Double Check**: Both `submitted` and the existence of an error must be true to display the message
...
### 2.7: Testing the Complete Validation Implementation
...
Now we need to add two validation functions: one for the new catch form and one for recorded catches.

Add these functions inside your `CatchLog` component, after the state declarations:
...
```jsx title="src/pages/CatchLog.jsx" showLineNumbers=131
...
```jsx title="src/pages/CatchLog.jsx" showLineNumbers=154
const handleSubmit = async (e) => {
  e.preventDefault();
  //diff-add-start
